...
pages.register =  async (req, res) => {
    ...
    if(!(req.files && req.files.portada && req.files.logo)) {
    ...
    portada = await upload(req.files.portada, 'pages/portada');
    logo = await upload(req.files.logo, 'pages/logos');
...
pages.update = async (req, res) => {
    ...
    if(req.files && req.files.portada && !req.files.logo) {
        ...
        const portadaFile = await upload(req.files.portada, 'pages/portada');    
        portada = portadaFile.fileName;   
        logo = page.rows[0].pagina_logo;
    }
    else if (req.files && req.files.logo && !req.files.portada) {
        removeFile(page.rows[0].pagina_logo, 'logos');
        const logoFile = await upload(req.files.logo, 'pages/logos');
...
pages.publish = async (req, res) => {    
    ...
    if(req.files && req.files.image) {
        image = await upload(req.files.image, 'pages/publication');    
    }
